...
// { view: 'all'|'groups'|'group-detail', group: string, filter: string, sort: string, clipId: string }
//
// URL scheme:
...
const DEFAULT_STATE = { view: 'all', group: '', filter: DEFAULT_FILTER, sort: '', clipId: '' }
...
export function parseUrl(pathname, search) {
  ...
  if (segments.length === 0) {
    return { view: 'all', group: '', filter: filter, sort: sort, clipId: '' }
  }
  if (segments[0] === 'clips' && segments.length === 2) {
    return { view: 'all', group: '', filter: filter, sort: sort, clipId: segments[1] }
  }
  if (segments[0] === 'groups') {
    if (segments.length === 1) {
      return { view: 'groups', group: '', filter: DEFAULT_FILTER, sort: '', clipId: '' }
    }
    if (segments.length === 2) {
      return { view: 'group-detail', group: segments[1], filter: DEFAULT_FILTER, sort: '', clipId: '' }
    }
    if (segments.length === 4 && segments[2] === 'clips') {
      return { view: 'group-detail', group: segments[1], filter: DEFAULT_FILTER, sort: '', clipId: segments[3] }
    }
  }
