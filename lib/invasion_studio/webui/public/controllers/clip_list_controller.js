...
export default class extends ApplicationController {
  ...
  async exportGroup() {
    ...
    try {
      let res = await this.requestExport(group, filename, false)
      ...
      if (res.status === 409 && data.overwrite_required) {
        ...
        if (!confirmed) return
        res = await this.requestExport(group, filename, true)
        data = await res.json()
      }
      if (res.ok) {
  ...
  requestExport(group, filename, overwrite) {
    return fetch('/api/export', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ group: group, output_basename: filename || null, overwrite: overwrite })
    })
  }
