...
export default class extends ApplicationController {
  static targets = ["container", "panelTitle", "filterDropdown", "filterSelect", "sortDropdown", "sortSelect", "groupExport", "uploadButton", "searchControls", "searchInput", "tagFilter", "ratingFilter", "resultFilter", "emptyTrashBtn", "clipCount", "clearFiltersBtn"]
  static values = {
  ...
  updateVisibility() {
    ...
    // bubble; the groups views show a plain title instead.
    if (this.viewValue === 'groups') {
  ...
  render() {
    ...
    if (this.clips.length === 0) {
      const message = this.viewValue === 'all'
        ...
        : 'Add clips to this compilation from "All Clips".'
    ...
    this.clips.forEach((clip) => {
      ...
      if (this.filterValue === 'deleted') {
        ...
        if (assignedGroups.length > 0) {
          ...
          const options = this.groups.map(g => '<option value="' + this.escapeAttribute(g.name) + '">' + this.escapeHtml(g.name) + '</option>').join('')
                groupActionHtml = '<select class="w-28 bg-surface-secondary text-text-primary border border-border px-1.5 py-0.5 rounded text-[11px] cursor-pointer"><option value="">Compilation...</option>' + options + '<option value="__new__">+ New Compilation...</option></select>'
        }
      } else {
      ...
      const dragHandle = (this.viewValue === 'group-detail' && this.filterValue !== 'deleted')
        ? '<span class="drag-handle text-text-muted cursor-grab px-1 select-none"><i data-lucide="arrow-down-up" class="size-4"></i></span>'
        : ''
  ...
  renderResultOverlay(result) {
    const overlays = {
      win: { icon: 'crown', classes: 'text-white', label: 'Win' },
      loss: { icon: 'skull', classes: 'text-white', label: 'Loss' },
