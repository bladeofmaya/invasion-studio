...
export default class extends ApplicationController {
  static targets = ["tab", "backBtn", "groupGrid", "previewPanel", "main"]
  static values = {
  ...
  updateVisibility() {
    ...
    if (this.hasMainTarget) {
      // The compilations view has no preview panel — let it span the full width
      this.mainTarget.style.gridTemplateColumns = this.currentViewValue === 'groups' ? '1fr' : ''
    }
  }

  dispatchStateChanged() {
    // document-level: listeners live on descendants of this element, which a
    // bubbling event dispatched here would never reach (bubbles go up).
